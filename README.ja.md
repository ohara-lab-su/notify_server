...
計算ジョブ終了時に、
HPC や共通アカウント側からメールを直接送らず、
自分の Mac 上で動く通知サーバーへ REST API で通知要求だけを送るための最小構成です。

計算サーバー側には、
メールアカウント、SMTP パスワード、アプリパスワードを置きません。
通知の実処理は、自分の Mac / 自分のユーザーアカウントで動く `notify_server.py` が行います。
